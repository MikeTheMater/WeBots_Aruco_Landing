...
class Mavic (Robot):
    ...
    def run(self):
            ...
            if altitude > self.target_altitude - 1:
                ...
                if self.getTime() - t1 > 0.1:
                    ...
                    if self.image is not None:
                        ...
                        image_array = np.frombuffer(self.image, dtype=np.uint8).reshape((self.camera.getHeight(), self.camera.getWidth(), 4))
                        ...
                        if ids is not None and target_id in ids:
                            ...
                            if abs(cx - self.camera.getWidth() / 2) < 50 and abs(cy - self.camera.getHeight() / 2) < 50:
                                # Land the drone
                                land()
        ...
        def land():
            MAX_DESCENT_VELOCITY = -10.0
            self.front_left_motor.setVelocity(MAX_DESCENT_VELOCITY)
            self.front_right_motor.setVelocity(MAX_DESCENT_VELOCITY)
            self.rear_left_motor.setVelocity(MAX_DESCENT_VELOCITY)
            self.rear_right_motor.setVelocity(MAX_DESCENT_VELOCITY)
            if altitude < 0.01:
                print("Landed")
